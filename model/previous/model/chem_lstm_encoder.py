...
class ChildSumTreeLSTMCell(nn.Module):
    def __init__(self, node_dim, edge_dim, h_size):
        ...
        self.W_xf = nn.Linear(node_dim+h_size, h_size, bias=False)

        # Input gate
        self.W_i = nn.Linear(node_dim + h_size, h_size)
        self.W_xi = nn.Linear(node_dim+h_size, h_size, bias=False)

        # Cell candidate (update)
        self.W_u = nn.Linear(node_dim + h_size, h_size)
        self.W_xu = nn.Linear(node_dim+h_size, h_size, bias=False)

        # Output gate
        self.W_o = nn.Linear(node_dim + h_size, h_size)
        self.W_xo = nn.Linear(node_dim+h_size, h_size, bias=False)
    ...
    def reduce_func(self, mailbox, mask):
        ...
        return {"h_sum": h3, "c": c_tilde, "x": h_sum}

    def apply_node_func(self, nodes):
        """
        Apply LSTM cell updates for the current node.
        """
        residual_h = nodes["h_sum"]
        residual_c = nodes["c"]
        residual_x = nodes["x"]
        # Input gate
        i = torch.sigmoid(self.W_i(residual_h)+self.W_xi(residual_x))
        # i = F.leaky_relu(self.W_i(nodes["h_sum"]) + self.b_i, negative_slope=0.01)
        
        # Cell candidate
        u = torch.tanh(self.W_u(residual_h)+self.W_xu(residual_x))
        # u = F.leaky_relu(self.W_u(nodes["h_sum"]) + self.b_u, negative_slope=0.01)
        
        # New cell state
        c = i * u + residual_c  # Update cell state
        
        # Output gate
        o = torch.sigmoid(self.W_o(residual_h)+self.W_xo(residual_x))
        # o = F.leaky_relu(self.W_o(nodes["h_sum"]) + self.b_o, negative_slope=0.01)
        
        # New hidden state
        h = o * (torch.tanh(c) + residual_c)
