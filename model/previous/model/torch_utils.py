...
def save_model(model, epoch, global_step, save_dir, optimizer = None, optimizer_info = None, extra_data=None, epoch_zero_fill=4, iter_zero_fill=8):
...
def load_model(func_create_model_form_config, load_dir, load_epoch, load_iter=None, device=None, extra_config_data=None, is_print=True):
    ...
    return model, epoch, global_step, optimizer, optimizer_info, extra_data
...
def create_subset_dataloader(dataloader, num_samples, batch_size, shuffle=True):
    """
    Create a new DataLoader with randomly selected `num_samples` from the given DataLoader.
    ...
    :param shuffle: Whether to shuffle data in the new DataLoader
    :return: A new DataLoader with a subset of the original dataset
    """
    ...
    # Create a subset dataset with selected indices
    subset_dataset = Subset(dataset, random_indices)
    
    # Create a new DataLoader with the subset
    new_dataloader = DataLoader(subset_dataset, batch_size=batch_size, shuffle=shuffle)
