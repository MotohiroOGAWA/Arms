...
class TsvLogger:
    def __init__(self, log_file_path, flush_interval=10, timestamp_format='%Y%m%d%H%M%S', extra_columns=None):
        """
        Initialize the TSVLogger class with buffered writing.

        Args:
            log_file_path (str): Base path to the log file where data will be saved in TSV format.
            flush_interval (int): Number of logs to buffer before writing to disk.
            timestamp_format (str): Format for the timestamp to be added to the file name.
        ...
        self.buffer = []  # Buffer to store log entries
        self.columns = [
            'global_step', 'epoch', 'type', 'data_size', 'timestamp',
            'learning_rate', 'target_name', 'loss', 'accuracy', 'criterion'
        ]
        
        if extra_columns:
            self.columns.extend(extra_columns)

        self._pre_columns_cnt = len(self.columns)

        self._initialize_log_file()

    def _initialize_log_file(self):
        """
        Ensure the TSV file exists and load existing columns.
        """
        if os.path.exists(self.log_file_path):
            existing_df = pd.read_csv(self.log_file_path, sep='\t')
            self.columns = list(existing_df.columns)  # Update columns from existing file
        else:
            self._write_header()

    def _write_header(self):
        """
        Write the header to the TSV file if it does not already exist.
        """
        df = pd.DataFrame(columns=self.columns)
        df.to_csv(self.log_file_path, sep='\t', index=False)
    

    def log(self, log_type, epoch, global_step, data_size, target_data, learning_rate=None, timestamp=None, extra_columns=None):
        """
        Logs the specified data to the buffer, flushing periodically.
        ...
        Args:
            ...
            target_data (dict): Dictionary containing target-specific data.
            learning_rate (float, optional): Learning rate.
            timestamp (str, optional): Log timestamp.
            extra_columns (dict, optional): Additional columns.
        """
        ...
        row = {
            'global_step': global_step, 'epoch': epoch, 'type': log_type, 'data_size': data_size,
            'timestamp': timestamp, 'learning_rate': learning_rate if learning_rate is not None else 'N/A',
            'target_name': ','.join(target_names), 'loss': ','.join(map(str, loss_values)),
            'accuracy': ','.join(map(str, accuracy_values)), 'criterion': ','.join(criterions)
        }
    ...
    def flush(self):
        """
        Write buffered logs to the TSV file and clear the buffer.
        """
        if not self.buffer:
        ...
        if len(new_df.columns) != self._pre_columns_cnt:
            self._update_columns(new_df)

        # Append new data to the log file
        new_df.to_csv(self.log_file_path, sep='\t', mode='a', index=False, header=False)

        # Clear buffer
        self.buffer = []

    def _update_columns(self, new_df):
        """
        Check if new columns exist, and if so, update the existing TSV file.

        Args:
            new_df (pd.DataFrame): The new data entry DataFrame.
        """
        # Load existing data
        if os.path.exists(self.log_file_path):
            existing_df = pd.read_csv(self.log_file_path, sep='\t')
        ...
        all_columns = sorted(set(existing_df.columns) | set(new_columns))

        # If new columns are detected, update the entire log
        if set(existing_df.columns) != set(all_columns):
            ...
            # Save the updated log with new columns
            updated_df = pd.concat([existing_df, new_df], ignore_index=True)
            updated_df.to_csv(self.log_file_path, sep='\t', index=False)
    ...
    def __del__(self):
        """
        Destructor to ensure that all buffered logs are written before object deletion.
        """
        self.flush()
