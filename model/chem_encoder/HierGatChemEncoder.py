...
from ..utils.gat.GatLayer import GATLayer
from ..utils.gat.GatEncoder import GatEncoderBlock



class GatChemEncoder(nn.Module):
    def __init__(self, num_layers, node_dim, edge_dim, hidden_dim, num_heads, ff_dim, dropout=0.1, T=1.0, use_shared_block=False):
        ...
        """
        super(GatChemEncoder, self).__init__()
    ...
    def forward(self, nodes, edge_index, edge_attr, node_mask, edge_mask):
        """
        Forward pass of the Hierarchical GAT Encoder.

        Args:
            nodes (torch.Tensor): Input node features (batch_size, seq_len, node_dim).
        ...
        """
        nodes = self.node_proj_linear(nodes)  # Shape: [batch_size, seq_len, hidden_dim]
        h = nodes
        if self.use_shared_block:
            ...
            for _ in range(self.num_layers):  # Loop for the specified number of layers
                h = self.shared_block(nodes, h, edge_index, edge_attr, node_mask, edge_mask)
        else:
            # Use a different `GatEncoderBlock` for each layer
            for layer in self.layers:
                h = layer(nodes, h, edge_index, edge_attr, node_mask, edge_mask)
