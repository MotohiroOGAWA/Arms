...
def draw_graph(G, ax=None, edge_labels=False, save_path=None):
    if not ax:
        ax = plt.gca()
    pos = nx.spring_layout(
        G,
        seed=42,    # 毎回同じ結果にしたいとき（任意）
        k=1.0,      # ノード間の理想距離（大きくすると広がる）
        scale=20.0,  # 全体のスケーリング
        iterations=100  # レイアウト計算の反復回数（多いほど精密）
    )
...
if __name__ == "__main__":
    ...
    # グラフを描画して可視確認
    draw_graph(G, save_path="benzene_graph.png")

    # テスト（構造確認用）
    assert G.number_of_nodes() == 6, "ベンゼンのノード数は6のはず"
    assert G.number_of_edges() == 6, "ベンゼンのエッジ数は6のはず"

    print("✅ draw_graph test completed — check the plot.")